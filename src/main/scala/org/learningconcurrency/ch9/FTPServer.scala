...
object FTPServer extends App {
  ...
  def fileSystemEvents(directory: String):Observable[FileEvent] = {
    Observable.create { observer =>
      ...
      val fileLis = new FileAlterationListenerAdaptor {
        override def onFileCreate(file: File) =
          observer.onNext(FileCreated(file.getPath))
        override def onFileChange(file: File) =
          observer.onNext(FileModified(file.getPath))
        override def onFileDelete(file: File) =
          observer.onNext(FileDeleted(file.getPath))
      }
